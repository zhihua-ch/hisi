...
HI_S32 SAMPLE_COMM_VPSS_Start(HI_S32 s32GrpCnt, SIZE_S *pstSize, HI_S32 s32ChnCnt,VPSS_GRP_ATTR_S *pstVpssGrpAttr)
{
   ...
   // for(i=0; i<s32GrpCnt; i++)
    //{
        ...
        {
            SAMPLE_PRT("HI_MPI_VPSS_CreateGrp failed with %#x!,VpssGrp=%d\n", s32Ret,VpssGrp);
            return HI_FAILURE;
        }
        ...
        for(j=0; j<s32ChnCnt; j++)
        {
    		...
    		s32Ret = HI_MPI_VPSS_GetChnNrParam(VpssGrp, VpssChn,&pstChnNrParam);
			if (s32Ret != HI_SUCCESS)
            {
                SAMPLE_PRT("HI_MPI_VPSS_GetChnNrParam failed with %#x\n", s32Ret);
                return HI_FAILURE;
            }
			pstChnNrParam.u32SfStrength = 10;
			s32Ret = HI_MPI_VPSS_SetChnNrParam(VpssGrp, VpssChn,&pstChnNrParam);
			if (s32Ret != HI_SUCCESS)
            {
                SAMPLE_PRT("HI_MPI_VPSS_SetChnNrParam failed with %#x\n", s32Ret);
                return HI_FAILURE;
            }
